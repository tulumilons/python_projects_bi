...
# Reads columns E and K from File A, treating column E as string
file_a = pd.read_excel(
    file_a_path,
    usecols="E,Q", #change column based on need, for July K, for August N
    dtype={"E": "string"},
    engine="openpyxl"
)
file_a.columns = ["SKU_RAW", "VALUE_Q"] # Renames columns for clarity
...
sku_dict = dict(zip(file_a["SKU_KEY"], file_a["VALUE_Q"]))
...
for sheet_name in sheet_names:
    ...
    for start in range(1, n_rows, block_size):
        ...
        target_row = first_idx + 2  # Calculates target row (2 rows below SKU position)
        target_col = 11  # Column K, change as needed based on the month

        # If SKU matches, write value to column I
